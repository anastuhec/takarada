...
def E_analytic(K, rho, phys_parameters):
    b, t, t_, t12, epsilon, Vb, Vc, _ = phys_parameters
    Nk = len(K)
...
def h_k(k, rho, delta_b, delta_c, phys_parameters, eps0, include_hartree):
    
    _, t, t_, t12, epsilon, Vb, Vc, delta = phys_parameters
...
def diagonalize(rho, K, T, mu, phys_parameters, eps0, include_hartree):
    _, _, _, _, _, Vb, Vc, _ = phys_parameters
    Nk = len(K)
    ...
    for i in [0, Nk//2]:
        en, v = LA.eigh(h_k(K[i], rho, delta_b, delta_c, phys_parameters, eps0, include_hartree))
        energije[:,i] = en
        vecs[:,:,i] = v
        if T == 0:
...
class model:
    ...
    def GS(self):
        _, _, _, _, _, Vb, Vc, _ = self.phys_parameters
    ...
    def run2(self, betas, stops, Gamma, Nomega, eps):
        for i, beta in enumerate(betas):
            ...
            if i not in stops:
                ...
                m1 = mf_matrix1(self.K, self.rho, phys_parameters, self.mu)
                m1_tilde = np.einsum('ijx,jlx,mlx -> imx', self.vecs, m1, self.vecs.conj())
                m2 = mf_matrix2(self.K, self.rho, phys_parameters, self.mu)
                m2_tilde = np.einsum('ijx,jlx,mlx -> imx', self.vecs, m2, self.vecs.conj())
                m3 = mf_matrix3(self.K, self.rho, phys_parameters, self.mu)
                m3_tilde = np.einsum('ijx,jlx,mlx -> imx', self.vecs, m3, self.vecs.conj())
                m4 = mf_matrix4(self.K, self.rho, phys_parameters, self.mu)
...
def phi_mf(K, rho, U, energije, tok, phys_parameters, mu, omegas, Gamma):
    Nk = len(K)
    b, t, t_, t12, epsilon, Vb, Vc, _ = phys_parameters
    ...
    j_mf[0,0,:] = -2*t*np.sin(K) * (2*t*np.cos(K) - epsilon - mu)
    j_mf[1,1,:] = 2*t_*np.sin(K) * (-2*t_*np.cos(K) + epsilon - mu)

    phimf = np.zeros((len(omegas), Nk), dtype=np.complex128)
    for i, omega in enumerate(omegas):
        for ii, _ in enumerate(K):
            u = U[:,:,ii]
            tok_k = u @ tok[:,:,ii] @ u.conj().T
            jq_k = u @ j_mf[:,:,ii] @ u.conj().T
            A = spektralna_k(omega, mu, energije[:,ii], Gamma=Gamma)
            phimf[i, ii] += np.trace(jq_k @ A @ tok_k @ A) 
...
def rho_operators(K, phys_parameters):
    _, _, _, _, _, Vb, Vc, _ = phys_parameters
    thetas = - np.hstack([np.ones(2) * Vb/2, np.ones(2) * Vc/2])

    deltas = [0, 1]
    nus = [1, 2]

    rhos = np.zeros((4, 2, 2, len(K)), dtype=np.complex128)
...
K0s, K1s, K1bs =  np.zeros((10,5)), np.zeros((10,5)), np.zeros((10,5))
...
mu0 = find_GS_mu(Nk, mu0, phys_parameters, parameters1, parameters2, include_hartree, beta0=100, beta1=95)
print('f')
m = model(Nk, mu0, phys_parameters, parameters1, parameters2, include_hartree)
m.GS()
...
scale = 1.008
beta0 = 50
betas = beta0/scale**np.arange(200)

Ts = 1/np.array(betas)
ends = [int(np.emath.logn(scale, beta0/beta)) for beta in betas][2::2]
...
tok_tilde = np.einsum('ijx,jlx,mlx -> imx', m.vecs, tok, m.vecs.conj())
phi = phi_Kubo(m.K, tok_tilde, tok_tilde, spektralka, omegas)
np.save('spektralka.npy', spektralka)
np.save('tok_tilde.npy', tok_tilde)
np.save('phi.npy', np.hstack([omegas, phi]))

m.run2(betas, ends, Gamma, Nomega, eps)

plt.plot(m.Ts, m.delta_bs/m.delta_bs[0])
plt.plot(m.Ts, m.delta_cs/m.delta_cs[0])
plt.show()
...
np.save('K0.npy', m.K0)
np.save('K1.npy', m.K1)
np.save('K0_b.npy', m.K0_b)
np.save('K1_b.npy', m.K1_b)
np.save('K1q.npy', m.K1q)
np.save('mus.npy', m.mus)
plt.plot(m.Ts, m.mus)
plt.show()
print('done')
